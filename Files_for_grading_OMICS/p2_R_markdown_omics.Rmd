...
gene_counts <- plotCounts(dds, gene_id, intgroup = "diagnosis", returnData = TRUE)
print(gene_c
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("sig", "Significance", c("All", levels(df.plot.gene$sig))),
...
server <- function(input, output) {
  # Reactive dataframe based on significance filter
  df.shiny <- reactive({
    if (input$sig != "All") {
      df.plot.gene[df.plot.gene$sig == input$sig, ]
    } else {
      df.plot.gene
    }
  })
  ...
  df.click <- reactive({
    if (length(input$table_rows_selected)) {
      df.shiny()[input$table_rows_selected, ]
    } else {
